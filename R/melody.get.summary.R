...
melody.get.summary <- function(null.obj,
  ...
  for(d in study.ID){
    ...
    if(nrow(covariate.interest[[d]]) != (nrow(null.obj[[d]]$N) + length(null.obj[[d]]$rm.sample.idx))){
      ...
      rownames(covariate.interest[[d]]) <- rownames(null.obj[[d]]$N)
    }

    if(is.null(cluster[[d]])){
      cluster.nm <- rownames(null.obj[[d]]$N)
      names(cluster.nm) <- rownames(null.obj[[d]]$N)
      SUB.id[[d]] <- cluster.nm
    }else{
      if(!is.vector(cluster[[d]])){
        stop("cluster is not a list of vectors. \n")
      }
      if(length(cluster[[d]]) != (nrow(null.obj[[d]]$N) + length(null.obj[[d]]$rm.sample.idx))){
