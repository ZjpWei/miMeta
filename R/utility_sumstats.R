...
reg.fit.wald = function(Melody, SUB.id, filter.threshold = 0, ref = NULL, parallel.core = NULL, verbose = FALSE){
  ...
  if(verbose){
    message(parallel.core[1], " cores are used for generating summary statistics.")
  }
...
GetGlm.wald <- function(data.beta, X.idx){
  ...
  colnames(est.single) <- colnames(data.beta$Y)[1:(K-1)]
...
get_ridge_sumstat_wald <- function(data.beta, summarys, G, shrehold, cov.type = "ridge", verbose = FALSE){
  ...
  if(cov.type == "ridge"){
    ...
    if(verbose){
      message("Tuning ridge parameter : ",lambda, ".\n")
    }
