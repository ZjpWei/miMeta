...
melody.meta.summary <- function(Melody,
  ...
  tune.set <- list()
  GIC.tau <- c()
  if((is.null(tune.size.sequence) & tune.path == "sequence") | (is.null(tune.size.range) & tune.path == "gsection")){
  ...
  if(tune.path == "sequence"){
    ...
    # test.model <- melody.get.test(Melody.model = min.result, Melody = Melody, p.adjust.methods = "fdr")
    if(ouput.best.one){
    ...
    g.sequence <- c()
    g.id <- 1
    s.1.lambda <- support.sizes[1]
    s.2.lambda <- support.sizes[length(support.sizes)]
    ...
    for(s.lambda in c(s.1.lambda, s.left.lambda, s.right.lambda, s.2.lambda)){
      tmp.subset <- search.subset.s(Melody = Melody, L = L,
                                    quantile_sm = quantile_sm,
                                    s.size = s.lambda,
                                    tmp.result = tmp.result,
                                    search.loc = search.loc,
